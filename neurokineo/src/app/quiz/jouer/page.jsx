...
function QuizJouer() {
	...
	const initialiserQuiz = useCallback(() => {
		if (!chapitre || !region || !QUESTIONS[chapitre]) {
			router.push("/quiz");
			return;
		}
		...
		const melange = [...qs]
			...
			.map((q) => {
				const repAvecIndex = q.reponses.map((rep, i) => ({
					rep,
					estBonne: i === q.bonne_reponse,
				}));
				repAvecIndex.sort(() => Math.random() - 0.5);
				return {
					...q,
					reponses: repAvecIndex.map((r) => r.rep),
					bonne_reponse: repAvecIndex.findIndex((r) => r.estBonne),
				};
			});
				...
				<div className="flex flex-col gap-3 mb-4">
					{q.reponses.map((rep, i) => {
						...
						if (valide) {
							...
							style =
								"bg-white border-2 border-slate-200 text-slate-700 hover:border-[#1a6b8a] hover:bg-[#e8f4f8]";
						}
