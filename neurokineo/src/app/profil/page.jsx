...
export default function ProfilPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <Navbar showBack title="Mon profil" />

      <main className="flex-1 p-4 max-w-lg mx-auto w-full pt-6">
        <div className="flex flex-col items-center mb-6">
          <div className="w-20 h-20 bg-[#1a6b8a] rounded-full flex items-center justify-center text-3xl mb-3">👤</div>
          <p className="font-extrabold text-slate-800 text-lg">{prenom} {nom}</p>
          <p className="text-slate-400 text-sm">{profil?.email}</p>
          <span className="mt-2 text-xs bg-[#e8f4f8] text-[#1a6b8a] font-bold px-3 py-1 rounded-full">{annee}</span>
        </div>

        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 mb-4">
          <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wide mb-4">Informations personnelles</h2>

          <div className="flex gap-3 mb-3">
            <div className="flex-1">
              <label htmlFor="nom" className="text-xs font-bold text-slate-400 mb-1 block">Nom</label>
              <input id="nom" type="text" value={nom} onChange={e => setNom(e.target.value)} className={inputClass} />
            </div>
            <div className="flex-1">
              <label htmlFor="prenom" className="text-xs font-bold text-slate-400 mb-1 block">Prénom</label>
              <input id="prenom" type="text" value={prenom} onChange={e => setPrenom(e.target.value)} className={inputClass} />
            </div>
          </div>

          <div className="mb-3">
            <label htmlFor="date-naissance" className="text-xs font-bold text-slate-400 mb-1 block">Date de naissance</label>
            <input id="date-naissance" type="date" value={dateNaissance} onChange={e => setDateNaissance(e.target.value)} className={inputClass} />
          </div>

          <div className="mb-3">
            <label htmlFor="annee" className="text-xs font-bold text-slate-400 mb-1 block">Année de formation</label>
            <select id="annee" value={annee} onChange={e => setAnnee(e.target.value)} className={inputClass}>
          ...
          <div className="mb-1">
            ...
            <input id="ecole" type="text" value={ecole} onChange={e => setEcole(e.target.value)} className={inputClass} />
          </div>
        </div>

        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 mb-4">
          <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wide mb-4">Compte</h2>
          <label htmlFor="email" className="text-xs font-bold text-slate-400 mb-1 block">Email</label>
          <input id="email" type="email" value={profil?.email || ''} disabled
        ...
        <button type="button" onClick={sauvegarder} disabled={sauvegarde}
