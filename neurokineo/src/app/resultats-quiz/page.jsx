...
export default function ResultatsQuizPage() {
  ...
  const message = pct === 100 ? '🏆 Parfait !'
    : pct >= 75 ? '👏 Très bien !'
    : pct >= 50 ? '📚 Pas mal !'
    : '💪 Continue à réviser !'

  const couleurBarre = pct >= 75 ? 'bg-green-400'
    : pct >= 50 ? 'bg-amber-400'
    : 'bg-red-400'
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <nav className="bg-white border-b border-slate-200 px-4 h-14 flex items-center justify-center sticky top-0 z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="font-extrabold text-[#1a6b8a] text-base">Résultats</span>
        </div>
      </nav>
      ...
      <main className="flex-1 p-4 max-w-lg mx-auto w-full pt-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 text-center mb-4">
          <p className="text-5xl font-extrabold text-[#1a6b8a] mb-1">{score}/{total}</p>
          <p className="text-slate-400 text-sm mb-1">{pct}% de bonnes réponses</p>
          ...
          {reponses.map((r, i) => (
            ...
            >
              <div className="flex items-start gap-3">
                <span className={`text-sm font-bold flex-shrink-0 ${r.correct ? 'text-green-500' : 'text-red-500'}`}>
                  {r.correct ? '✓' : '✗'}
                </span>
                <div className="flex-1">
                  ...
                  {!r.correct && (
                    <p className="text-xs text-slate-400">
                      Ta réponse : <span className="text-red-500 font-bold">{r.reponses[r.reponseChoisie]}</span>
                      <br />
                      Bonne réponse : <span className="text-green-600 font-bold">{r.reponses[r.bonne_reponse]}</span>
                    </p>
                  )}
