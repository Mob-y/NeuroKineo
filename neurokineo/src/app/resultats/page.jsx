...
export default function ResultatsPage() {
  ...
  const chargerHistorique = useCallback(async () => {
    ...
    const { data } = await supabase.from('historique_quiz').select('*')
  ...
  const historiqueFiltré = filtre === 'tous' ? historique : historique.filter(r => r.matiere === filtre)
  ...
  function bgScore(score, total) {
    const pct = score / total
    if (pct >= 0.75) return 'bg-white border-green-200'
    if (pct >= 0.5) return 'bg-white border-amber-200'
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <Navbar showBack title="Vos résultats" />
      ...
      <main className="flex-1 p-4 max-w-lg mx-auto w-full pt-6">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className={`rounded-2xl border shadow-sm p-4 text-center ${moyenneGlobale >= 75 ? 'bg-green-50 border-green-200' : moyenneGlobale >= 50 ? 'bg-amber-50 border-amber-200' : 'bg-red-50 border-red-200'}`}>
            <p className={`text-2xl font-extrabold ${moyenneGlobale >= 75 ? 'text-green-500' : moyenneGlobale >= 50 ? 'text-amber-500' : 'text-red-500'}`}>{moyenneGlobale}%</p>
        ...
        {totalQuiz === 0 ? (
          ...
          <>
            <div className="flex gap-2 overflow-x-auto pb-2 mb-4">
              {matieres.map(m => (
                <button key={m} type="button" onClick={() => setFiltre(m)}
                  className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border-none cursor-pointer transition-colors ${filtre === m ? 'bg-[#1a6b8a] text-white' : 'bg-white text-slate-500 border border-slate-200'}`}>
                  {m === 'tous' ? 'Tous' : m}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-3">
              {historiqueFiltré.map(r => (
                <div key={r.id} className={`rounded-2xl border shadow-sm p-4 ${bgScore(r.score, r.total)}`}>
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      <p className={`text-xl font-extrabold ${couleurScore(r.score, r.total)}`}>{r.score}/{r.total}</p>
                      <p className={`text-xs font-bold ${couleurScore(r.score, r.total)}`}>{Math.round((r.score / r.total) * 100)}%</p>
                    </div>
                  </div>
                  <div className="bg-slate-100 rounded-full h-1.5 mt-3 overflow-hidden">
                    <div className={`h-full rounded-full ${(r.score / r.total) >= 0.75 ? 'bg-green-400' : (r.score / r.total) >= 0.5 ? 'bg-amber-400' : 'bg-red-400'}`}
